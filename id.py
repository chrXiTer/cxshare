...
def genIdHY(lastId):
    i1 = lastId[0:6]
    i2 = int(lastId[6:10])   #Y
    i3 = int(lastId[10:12])  #M
    i4 = int(lastId[12:14])  #D
    i5 = int(lastId[14:17])
...
def getRightId(id): #如果id不对，转换成正确的id。
    i6 = getIdLastChar(id)
    id = id[0:-1] + i6
    return id

def getLastIdLastYear(id): # 获与一个身份证件号同地区，前一年的最后一个身份证号（随机码为0）
    i1 = id[0:6]
    i2 = int(id[6:10])   #Y
    i2 = i2 -1
    i3 = 12
